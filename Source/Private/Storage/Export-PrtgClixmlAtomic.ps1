function Export-PrtgClixmlAtomic {
  ...
  .DESCRIPTION
    Export-Clixml truncates its target before it writes, so a write that fails part-way
    destroys the existing content. The payload goes to a pre-created temp file in the same
    folder and is swapped in as a single step, leaving nothing partial on disk. Names are
    prefixed 'PrtgWrite' so they cannot shadow the caller's hooks (ADR 0001).
  .PARAMETER PrtgWriteBeforeWrite
    Runs on the temp file before the payload goes into it, with that path as its argument.
  .PARAMETER PrtgWriteAfterSwap
    Runs on the destination after the swap, with that path as its argument.
  #>
  ...
  try {
    ...
    # folder may contain. A .NET create resolves a relative path against the process CWD.
