function Clear-PrtgSensorState {
  ...
  $action = {
    ...
    $loaded = Get-PrtgStateEntry -File $file -Cmdlet 'Clear-PrtgSensorState' -Noun 'state file' `
      -UnreadableConsequence ' and will be deleted'
    $entries = @($loaded)
  ...
  # DeleteOnClose: the OS removes the sidecar when the handle closes, so a live holder
  # never loses it.
